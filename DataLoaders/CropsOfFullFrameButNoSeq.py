...
class CropsOfFullFrameButNoSeq(Dataset):
    def __init__(self, annots, frames_annots_path, transform, matches, level = 'person_level'):
        """
        Args:
    ...
    def extract_crops(self, frame, boxes = []):
        ...
        if self.level == 'group_level':
            ...
            if len(crops) > 12 :
                crops = crops[:12]
        return torch.stack(crops)                            # 1 x 3 x 244 x 244 or 12 x 3 x 244 x 244
    ...
    def __getitem__(self, idx):
        ...
        crops = self.extract_crops(frame, sample['boxes']).squeeze(0)

        if self.level == 'group_level':
            label = torch.tensor(self.group_activity_labels[sample['category']], dtype=torch.long)  
        else :
            label = torch.tensor(self.person_activity_labels[sample['category']], dtype=torch.long) 
            
    
        return crops, label   # tensor with shape (3, H, W)   ,    scalar tensor  
