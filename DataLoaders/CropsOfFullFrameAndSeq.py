...
class CropsOfFullFrameAndSeq(Dataset):
    def __init__(self, annots, frames_annots_path, transform, matches:List[int], level:str = 'person_level', seq:bool=True,sort:bool = False):
        """
        Args:
    ...
    def extract_crops(self, frame, boxes = []):
        ...
        for box in boxes :
            ...
            if self.level == 'group_level' and self.sort : 
        ...
        return torch.stack(crops), None                            #  12 x 3 x 244 x 244
    ...
    def __getitem__(self, idx):
        ...
        if self.level == 'person_level': 
            ...
            if self.seq : players_categories = players_categories[:,-1]              # 12
                                                                        # If not seq like B6_A --> # 12,9
            
        else : label = torch.tensor(self.group_activity_labels[sample['clip_category']], dtype=torch.long) 

        if self.level == 'person_level' :
            return crops, players_categories   # 9,12,3,244,244     12,1 or 12,9
        else :
            return crops, label