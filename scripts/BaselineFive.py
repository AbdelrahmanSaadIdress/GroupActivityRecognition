...
from networks import OneCropNoSeqModel
from networks import SeqCropsModel, ClipsModel


def prepare_the_run_B5_P1(config, B3_CP_path):
    ...
    if B3_CP_path:
        model_path = B3_CP_path
        load_checkpoint(config, model1, None, None, None, False, model_path )
    model = SeqCropsModel(  model1 , config['Modelling']['num_classes']).to(device)
...
def prepare_the_run_B5_P2(config ,B5_CP_A_path):
    ...
    model1 = SeqCropsModel().to(device)
    if B5_CP_A_path:
        model_path = B5_CP_A_path
        load_checkpoint(config, model1, None, None, None, False, path=model_path )

    model = ClipsModel( model1 , config['Modelling']['num_classes']).to(device)
