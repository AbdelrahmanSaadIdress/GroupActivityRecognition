...
from networks import FullModelWithNoSorting, FeatureExtractor



def prepare_the_run_B7_P1(config):
    ...
    model = FeatureExtractor( config['Modelling']['num_classes']).to(device)
...
def prepare_the_run_B7_P2(config ,B5_CP_A_path):
    ...
    model = FullModelWithNoSorting( model1 ).to(device)
