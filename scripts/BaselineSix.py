...
from networks import CropsModel, SeqFramesModel


def prepare_the_run_B6_P1(config, B3_CP_path):
    ...
    model = CropsModel(  model1 , config['Modelling']['num_classes']).to(device)
...
def prepare_the_run_B6_P2(config ,B6_CP_A_path):
    ...
    model1 = CropsModel().to(device)
    if B6_CP_A_path:
        model_path = B6_CP_A_path
        load_checkpoint(config, model1, None, None, None, False, path=model_path )

    model = SeqFramesModel( model1 , config['Modelling']['num_classes']).to(device)
