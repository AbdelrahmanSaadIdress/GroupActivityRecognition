...
from networks import SeqFramesModel_V1, SeqFramesModel_V2, FramesModel


def prepare_the_run_B4_V1(config, ck_path):
    ...
    model1 = FramesModel().to(device)
    if ck_path:
        model_path = ck_path
        load_checkpoint(config, model1, None, False, model_path )

    model = SeqFramesModel_V1(model1, config['Modelling']['num_classes']).to(device)
...
def prepare_the_run_B4_V2(config, ck_path):
    ...
    model = SeqFramesModel_V2(model1, config['Modelling']['num_classes']).to(device)
